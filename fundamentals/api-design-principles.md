...
**Solution**: Client generates a unique `Idempotency-Key: UUID`. Server stores this key. If it sees the same key again, it returns the *cached previous response* instead of executing the charge again.
