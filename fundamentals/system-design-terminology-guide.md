...
_A Practical Reference for Backend Engineers_

## 1. What Is System Design (In Simple Terms)

**System Design** is simply "Architecture planning."

When you write code, you solve a problem in a _single file_ or function.
When you do System Design, you solve a problem by connecting _multiple computers_ (servers, databases, caches) together.
...
**Backend Example**: A REST API that checks the JWT in the header for _every_ request. It doesn't care if you logged in 5 minutes ago or 5 seconds ago; the token tells the truth. **Essential for scaling**.
...
**Definition**: A reference to a Primary Key in _another_ table. It links them validly.
...
**Why**: Without an index, finding "User with email `bob@gmail.com`" requires scanning 1 million rows. _With_ an index, it takes 1 millisecond.
...
**Definition**: If this one thing breaks, the _entire_ system stops working.
...
**Definition**: The guarantee that the message WILL be delivered, but it _might_ be delivered twice (e.g., network timeout on acknowledgment).
...
3.  **Read Engineering Blogs**.
    - Companies like Uber, Netflix, and Discord write blogs about _real_ problems.
    - _Task_: Read "How Discord stores billions of messages." You will see these terms (Cassandra, Latency, Cache) in action.
4.  **Practice "Why"**.
