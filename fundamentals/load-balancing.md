...
- _Logic_: 1, 2, 3, 1, 2, 3...
- _Best for_: Servers with equal specs and requests that take equal time.

### 2. Least Connections

- _Logic_: "Server 1 has 5 active users. Server 2 has 2 active users. Send next to Server 2."
- _Best for_: Long-lived connections (WebSockets, File Uploads).

### 3. IP Hash

- _Logic_: Hash the user's IP. Result is 2. Send to Server 2.
- _Feature_: "Sticky". The same user always reaches the same server. **Avoid relies on this for session state** (makes autoscaling hard).
...
A type of LB that sits in front of _one_ or _many_ servers.
**Functions**:
