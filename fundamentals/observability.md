...
- **Structured Logging**: Use JSON. `{ "level": "error", "userId": 123, "msg": "failed" }`.
  - _Why?_ Computers can search JSON (find all errors for User 123). They can't search text easily.
- **Levels**:
  - `INFO`: "User logged in".
...
_ Span A: Frontend -> Gateway (20ms).
_ Span B: Gateway -> Auth Service (50ms) [ERROR].
