...
- **Event Handling**: If the "UserRegistered" event fails to publish to Kafka (Broker down).
  - **Outbox Pattern**: We write the message to a local `outbox` table in Postgres _in the same transaction_ as creating the user.
  - A separate background process reads `outbox` and retries pushing to Kafka.
  - _Result_: Zero data loss. Eventual consistency guaranteed.
