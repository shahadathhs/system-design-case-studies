...
*   **Database Partitioning**:
    ...
    *   We likely *don't* need to shard `users` yet (Postgres handles 100M rows fine with indexes), but if needed, we shard by `user_id`.
*   **Geo-Replication**:
...
*   **Post-processing**: The API pushes a "UserRegistered" event to Kafka.
    ...
    *   *Why?* Keeps the Registration API response time fast (< 200ms).
