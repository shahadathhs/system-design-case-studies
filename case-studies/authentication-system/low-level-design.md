...
- **HS256**: Uses a single secret key. Both the Auth Service (signer) and Product Service (verifier) need the secret to verify the token.
  - _Risk_: If the Product Service is compromised, the attacker can Mint tokens.
- **RS256**: Uses a Private/Public key pair.
  ...
  - _Benefit_: Even if a microservice is hacked, the attacker cannot forge tokens.
...
- _Default_: Reuse detection triggers, banning the user.
- _Fix_: Implement a "Grace Period" of 10-30 seconds where the old `RT` is still accepted but returns the _same_ new `RT_2` instead of rotating again.
